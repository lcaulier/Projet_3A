...
def main(csv_filepath,size_image_output):
    ...
    while x <limit or y<limit or z<limit:
        ...
        if not pd.isna(data.iloc[0,79]) and x<limit:
            filepath = os.path.join(path_foldername_front, "front_"+str(prod_id)+".jpg")
            rep = requests.get(data.iloc[0,79])
            with open(filepath, "wb") as f:
                f.write(rep.content)
            x+=1
        if not pd.isna(data.iloc[0,81]) and y<limit:
            filepath = os.path.join(path_foldername_ingredients, "ingredients_"+str(prod_id)+".jpg")
            rep = requests.get(data.iloc[0,81])
            with open(filepath, "wb") as f:
                f.write(rep.content)
            y+=1
        if not pd.isna(data.iloc[0,83]) and z<limit:
            filepath = os.path.join(path_foldername_nutrition, "nutrition_"+str(prod_id)+".jpg")
    ...
    for filename in foldername_front:
        front_arrays.append(img_to_array(load_img(path_foldername_front +'/'+ filename, target_size=(size_image_output, size_image_output))))
    
    nutrition_arrays = []
    foldername_nutrition = os.listdir(path_foldername_nutrition)
    for filename in foldername_nutrition:
        nutrition_arrays.append(img_to_array(load_img(path_foldername_nutrition +'/'+ filename, target_size=(size_image_output, size_image_output))))
    
    ingredients_arrays = []
    foldername_ingredients = os.listdir(path_foldername_ingredients)
    for filename in foldername_ingredients:
        ingredients_arrays.append(img_to_array(load_img(path_foldername_ingredients +'/'+ filename, target_size=(size_image_output, size_image_output))))
    ...
    with open("data/processed/array_input_"+str(size_image_output)+".npy", 'wb') as f:
        np.save(f, X)

    with open("data/processed/array_output_"+str(size_image_output)+".npy", 'wb') as f:
        np.save(f, y)
